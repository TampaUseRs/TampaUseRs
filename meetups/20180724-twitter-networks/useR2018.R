...
#'
#' The current new "state of the art" for retweeting and manipulating twitter
#' data in the R world is [rtweet](http://rtweet.info/), which is what we'll use
#' today. It has a collection of good introducory vignettes if you'd like to
#' learn more.
#'
#' The basic outline of code and analysis will be a mishmash of Mike Kearney's
#' [Ruser2018 analysis](https://github.com/mkearney/rstudioconf_tweets) and my
#' old NIPS2016 analysis. Kearney's analysis is quite good, I recommend it for
...
deg_cutoff <- function(graph, q_cutoff = 0.95) {
  ...
  dfv <- cbind(dfv, quantile = cut(dfv$degree,
    breaks = quantile(dfv$degree, probs = c(0, q_cutoff, 1)),
...
# what can we do once we have clusters? go back to the rtweet data and see how
# different communities are talking about the topic. Do this by hand coding if
# data is small (yuck), or LDA (latent dirichlet allocation) for automated topic
# modeling

# This approach will lay out the network as a circle, which sometimes make it
# easier to see relationships when you have a high number of connections (high
# activity networks like useR, rstudioConf)
...
#' # Initial vis
#'
#' Time series of the data in two hour chunks. One REALLY nice thing about
#' rtweet is that it makes plotting the timeseries of tweet a completely
#' lazy-person's function, ts_plot, where you can feed it the aggregation time
#' to summarize over. Here, we go with 2 hour's as that seems like a good medium
#' to begin with.
#'
#'
## ----initvis-------------------------------------------------------------
...
p <- rt %>%
  ...
  labs(
    title = "Frequency of tweets about ruser2018 over time",
...
#'
#'
#' Again, another analysis I've done in the past, but here I'll use Mike
#' Kearney's version simply to make my life a bit easier as it's mapping on to
#' rtweet's data, and I haven't actually used the syuzhet, which is a commonly
#' used sentiment analysis package.
#'
#' I could and probably should be doing even more involved text cleanup than
#' this.
#'
## ----sentiment-----------------------------------------------------------
...
#' # Influence (dubious)
#'
#' Here is that somewhat (very) dumb influence metric I cooked up or adapted
#' from elsewhere, I can't quite remember at this point. Either way I don't put
#' much value in it. It's basically just the sum of favorites and retweets
#'
## ----influence-----------------------------------------------------------
...
# library(viridis)
showvals2 <- showvals %>%
  mutate(impact = fav_count + rt_count) %>%
  arrange(-impact)
...
#'
#' The word cloud gets a bad rap, I mean, it IS basically impossible to
#' intrepret in any numerical or comparative sense. But I do still find it
#' useful to get a quick overview of just what types of things people are
#' talking about. And thus, wordclouds we go.
#'
#'
## ----wordcloud-----------------------------------------------------------
...
#'
#' Won't have time to talk about these packages I think, but for those
#' interested, graphTweets automates the network making aspect and has some cool
#' dynamic graph aspects that most other packages don't have (or not well at
#' least)
#'
